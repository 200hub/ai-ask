...
#### ✅ 推荐格式（全平台兼容）
- `1.0.0` - 正式版本
- `1.0.0-1` - 预发布版本（纯数字标识符）
- `1.0.0-2` - 预发布版本（纯数字标识符）

#### ⚠️ 支持但需要自动转换的格式
- `1.0.0-alpha.1` → 自动转换为 `1.0.0-1`（Windows MSI）
- `1.0.0-beta.2` → 自动转换为 `1.0.0-2`（Windows MSI）
- `1.0.0-rc.3` → 自动转换为 `1.0.0-3`（Windows MSI）

**说明**：
- macOS 和 Linux 支持任意预发布标识符（如 `alpha`, `beta`, `rc`）
- Windows MSI 要求预发布标识符必须是纯数字且 ≤ 65535
- GitHub Actions 会**自动**在 Windows 构建时将 `x.y.z-word.n` 转换为 `x.y.z-n`
- 其他平台保持原始版本号不变

#### ❌ 不支持的格式
- `1.0.0-test` - 缺少版本号（无法提取数字）
- `1.0.0-alpha` - 缺少版本号（无法提取数字）
- `1.0.0-beta` - 缺少版本号（无法提取数字）
